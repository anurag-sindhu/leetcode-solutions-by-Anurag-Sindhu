...
var cutOffTree = function (forest) {
    ...
    const distFromLeftToRight = [];
    let colIndex = Infinity;
    for (let rowIndex = 0; rowIndex < forest.length; rowIndex++) {
        colIndex = Infinity;
        distFromLeftToRight[rowIndex] = [];
        for (let columnIndex = 0; columnIndex < forest[rowIndex].length; columnIndex++) {
            const element = forest[rowIndex][columnIndex];
            if (element == 0) {
                distFromLeftToRight[rowIndex].push(columnIndex - colIndex);
            } else {
                distFromLeftToRight[rowIndex].push(0);
                colIndex = columnIndex;
            }
        }
    }
    const distFromRightToLeft = [];
    colIndex = Infinity;
    for (let rowIndex = 0; rowIndex < forest.length; rowIndex++) {
        colIndex = Infinity;
        distFromRightToLeft[rowIndex] = [];
        for (let columnIndex = forest[rowIndex].length - 1; columnIndex >= 0; columnIndex--) {
            const element = forest[rowIndex][columnIndex];
            if (element == 0) {
                distFromRightToLeft[rowIndex][columnIndex] = colIndex - columnIndex;
            } else {
                distFromRightToLeft[rowIndex][columnIndex] = 0;
                colIndex = columnIndex;
            }
        }
    }

    const distFromTopToBottom = [];
    let rowInd = Infinity;
    for (let columnIndex = 0; columnIndex < forest[0].length; columnIndex++) {
        rowInd = Infinity;
        for (let rowIndex = 0; rowIndex < forest.length; rowIndex++) {
            const element = forest[rowIndex][columnIndex];
            if (!distFromTopToBottom[rowIndex]) {
                distFromTopToBottom[rowIndex] = [];
            }
            if (element == 0) {
                distFromTopToBottom[rowIndex][columnIndex] = rowIndex - rowInd;
            } else {
                distFromTopToBottom[rowIndex][columnIndex] = 0;
                rowInd = rowIndex;
            }
        }
    }
    const distFromBottomToTop = [];
    rowInd = Infinity;
    for (let columnIndex = 0; columnIndex < forest[0].length; columnIndex++) {
        rowInd = Infinity;
        for (let rowIndex = forest.length - 1; rowIndex >= 0; rowIndex--) {
            if (!distFromBottomToTop[rowIndex]) {
                distFromBottomToTop[rowIndex] = [];
            }
            const element = forest[rowIndex][columnIndex];
            if (element == 0) {
                distFromBottomToTop[rowIndex].push(rowInd - rowIndex);
            } else {
                distFromBottomToTop[rowIndex].push(0);
                rowInd = rowIndex;
            }
        }
    }
    ...
    for (const element of trees) {
        ...
        const path = {};
        path[`${lastRowIndex}_${lastColumnIndex}`] = true;
        const minSteps =
            1 +
            findSteps(
                forest,
