...
var constructFromPrePost = function (preorder, postorder) {
    ...
    if (preorder.length <= 3) {
        ...
        level = 3;
    } else if (preorder.length <= 15) {
        level = 4;
    } else {
        level = 5;
    }
    const indexAddressed = {};
    let runningLevel = level - 1;
    function buildTree(parentIndex = 0) {
        const child1 = parentIndex + 1;
        const child2 = parentIndex + Math.pow(2, runningLevel) - 1;
        if (indexAddressed[child1] && indexAddressed[child2]) {
            return;
        }
        if (!indexAddressed[child1] && preorder[child1] !== undefined) {
            indexAddressed[child1] = true;
            binary.add(preorder[child1]);
        }
        if (!indexAddressed[child2] && preorder[child2] !== undefined) {
            indexAddressed[child2] = true;
            binary.add(preorder[child2]);
        }
        runningLevel--;
        buildTree(child1);
        buildTree(child2);
    }
    binary.add(preorder[0]);
    buildTree();
    return;
};

console.log(
    constructFromPrePost((preorder = [1, 2, 4, 5, 3, 6, 7]), (postorder = [4, 5, 2, 6, 7, 3, 1])),
);
